...
void Repo::show(std::ostream& os) const {
    ...
    // HEAD branch
    os << "Head branch: " << head_branch_ << "\n";

    // HEAD commit
    if (head_) {
        os << "HEAD commit: " << head_->id
            << " (" << head_->timestamp << ")  "
            << head_->message << "\n";
    }
    else {
    ...
    for (const auto& branch : branches_) {
        os << "  " << branch.first << " -> ";
        if (branch.second) os << branch.second->id;
        if (branch.first == head_branch_) os << " [HEAD]";
        os << "\n";
    }
    os << "\n";
    ...
    for (const auto& branch : branches_) {
        auto commit = branch.second;
        while (commit) {
            if (printed.find(commit->id) == printed.end()) {
                counter++;
                commits_os << "  Commit: " << commit->id << "\n";
                commits_os << "    timestamp: " << commit->timestamp << "\n";
                commits_os << "    message: " << commit->message << "\n";
                commits_os << "    files:\n";
                for (const auto& file : commit->files) {
                    commits_os << "      " << file.first << " -> \""
                        << (file.second ? *file.second : "") << "\"\n";
                }
                printed.insert(commit->id);
            }
            else {
                break;
            }
